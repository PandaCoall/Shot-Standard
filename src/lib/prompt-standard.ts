...
- Each heading MUST be wrapped in square brackets on its own line. Never write SCENE: — write [SCENE]. Use these markers EXACTLY, in this order:

${markers}

- After each marker, write only the content. Never write heading instructions such as "Describe the location" or "Describe the main character".
...
- [SUBJECT] must begin with exactly one tag on its own line: [Man], [Woman], [Child], or [Officer]. Then describe the person/object AND what they are doing in enough detail to stay consistent across frames (age, hair, skin, clothes, expression, pose, gesture).
...
export function buildUserPrompt(input: {
  ...
  const lines = [
    "Write the MiniMax prompt plate for this still. Use bracket markers like [SCENE], not SCENE:.",
    "",
    `Subject override: ${
      input.subject === "auto"
