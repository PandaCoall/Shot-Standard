...
export const Route = createFileRoute("/api/generate")({
  ...
  server: {
    handlers: {
      ...
      GET: async () =>
        json({ ok: false, error: "Use POST with image or imageUrl." }, 405),
      POST: async ({ request }) => {
        try {
          ...
          try {
            if (contentType.includes("application/json") || !contentType) {
              ...
              const form = await request.formData();
              const file =
                form.get("image") ?? form.get("file") ?? form.get("data");
              if (file instanceof File) {
                ...
                payload = {
                  image: String(form.get("image") ?? "") || undefined,
                  imageUrl:
                    String(form.get("imageUrl") ?? form.get("url") ?? "") ||
                    undefined,
                };
              }
            } else {
            ...
            return json(
              { ok: false, error: "Body must be JSON or form data." },
              400,
            );
          }
          ...
          if (!still.ok) {
            return json({ ok: false, error: still.error }, still.status);
          }
